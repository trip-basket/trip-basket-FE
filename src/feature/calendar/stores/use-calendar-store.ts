...
/**
 * Create an array of TripDay objects for each date in the given range.
 *
 * @param startDate - Start date in `YYYY-MM-DD` format
 * @param endDate - End date in `YYYY-MM-DD` format
 * @returns An array of TripDay entries for each date from `startDate` to `endDate` (inclusive); each entry contains a localized `date` string, `dayOfWeek` label, numeric `dateNum`, and an empty `blocks` array
 */
...
/**
 * Create trip days for 2026-02-21 through 2026-03-01 and populate each day with mock calendar blocks.
 *
 * @returns An array of `TripDay` objects for the date range, with each day's `blocks` filled from `MOCK_CALENDAR_BLOCKS` where `dayIndex` matches.
 */
