...
export function AddDayButton({ label, position }: { label: string; position: "before" | "after" }) {
  ...
  return (
    <button
      type="button"
      className="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-muted rounded-lg border border-transparent hover:!bg-hover transition-colors cursor-pointer"
      onClick={handleClick}
    >
